...
class TestConfigShow:
    ...
    def test_shows_url_and_key_preview(self, config_file, capsys):
        ...
        opencdr.cmd_config_show(SimpleNamespace())
        out = capsys.readouterr().out
        # CodeQL flags this as incomplete URL substring sanitization -- that
        # query targets `if "trusted-domain" in url` used for access control,
        # where substring containment is exploitable. This is a test
        # assertion on printed CLI output, not a security boundary.
        assert "https://api.example.com" in out  # lgtm[py/incomplete-url-substring-sanitization]
        assert "abcdefgh" in out
        assert "abcdefghijklmnop" not in out
...
class TestLoadSaveConfigRoundTrip:
    ...
    def test_save_restricts_file_to_owner_only(self, config_file):
        opencdr._save_config({"url": "https://api.example.com", "key": "k"})
