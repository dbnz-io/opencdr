...
def _save_config(data: dict) -> None:
    # Real, not a suppression: CodeQL correctly flags this as storing a
    # credential (the API key) in clear text. Encrypting a local CLI
    # config file is out of scope (same accepted convention as aws-cli's
    # ~/.aws/credentials, gh's config.yml -- plaintext at rest, protected
    # by filesystem permissions rather than encryption), but restricting
    # those permissions to the owning user only, which this repo hadn't
    # done, is a real and cheap improvement. os.chmod after write, not
    # os.open with a mode, so this stays correct on the rewrite path too
...
def _die_on_error(status: int, body: Any, context: str = "") -> None:
    if status >= 400:
        ...
        # not a real finding: every api.py error message is a static,
        # generic description (e.g. "unknown response_module", "missing
...
def _run_setup_wizard() -> None:  # noqa: C901
    ...
    if channels:
        ...
        if s in (200, 201):
            ...
            # This one's worth being explicit about, given `channels` here
            # can carry real secrets (webhook URLs, auth headers) as
            # *input*: checked api.py's actual validation errors
            # (_normalize_settings_payload) directly rather than assuming
