...
- [Architecture](architecture.md) — the seven Lambdas, the data stores, and how an event flows from raw CloudTrail record to delivered alert
...
| What fields can a rule's `conditions` check? | [Detection Rules](detection-rules.md#signal-rules) |
...
| What can the single API key do — is it scoped? | [Security](security.md#known-deliberate-limitations) |
